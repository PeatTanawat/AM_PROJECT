...
try {
    ...
    // 1. เพิ่มคอลัมน์ is_auto_checked หากยังไม่มีในตาราง tbl_course_enrollment
    try {
        $db->exec("ALTER TABLE tbl_course_enrollment ADD COLUMN is_auto_checked VARCHAR(1) DEFAULT '0'");
    } catch (PDOException $e) {
        // คอลัมน์อาจจะเคยถูกเพิ่มไปแล้ว ข้ามข้อผิดพลาดนี้ไปได้
    }

    // 2. ค้นหารายการลงทะเบียนเรียนที่สอบผ่านแล้วเกิน 5 นาที แต่ยังไม่ได้อนุมัติ
    // และยังไม่เคยถูกตรวจสอบโดยระบบออโต้ (is_auto_checked = '0')
    $sql_pending = "SELECT e.enroll_id, e.enroll_user_id, e.enroll_course_id,
                    ...
                    WHERE e.enroll_is_completed = '0'
                      AND (e.is_auto_checked = '0' OR e.is_auto_checked IS NULL)
                      ...
                      AND EXISTS (
                          ...
                          WHERE a2.attempt_user_id = e.enroll_user_id 
                            AND a2.attempt_course_id = e.enroll_course_id 
                            AND a2.attempt_pass = '1'
                            AND a2.create_at <= NOW() - INTERVAL 5 MINUTE
                      )
                    LIMIT 5"; // จำกัดการรันแต่ละครั้งเพื่อไม่ให้ทำงานหนักเกินไป
    ...
    if (empty($pending_list)) {
        Response::json(1, 'ไม่มีรายการที่ต้องอนุมัติออโต้', null);
    }
    ...
    foreach ($pending_list as $row) {
        ...
        // เริ่ม Transaction สำหรับแต่ละรายการ เพื่อให้ทำงานแยกส่วนกันปลอดภัย
        $db->beginTransaction();
        ...
        try {
            ...
            // เงื่อนไข: ต้องมีเลข CPD หรือ CPA อย่างใดอย่างหนึ่งหรือทั้งคู่
            if ($user_info && ($has_cpd || $has_cpa)) {
                ...
                // 2. ออกใบรับรอง snapshot (รวมเป็น 1 เรคคอร์ด ไม่แยกใบ)
                ...
                if (!$snap_exists) {
                    ...
                    try {
                        ...
                        if ($ex->getCode() !== '23000') {
                            error_log('CronAutoApprove insert Error: ' . $ex->getMessage());
                        }
                    }
                }
            } else {
                // หากไม่มีเลข CPD/CPA จะถูกมาร์ก is_auto_checked = '1' ไว้แล้วตอนต้น และยังคง enroll_is_completed = '0' รอแอดมินอนุมัติมือ
            }
    ...
    Response::json(1, 'ดำเนินการอนุมัติสำเร็จจำนวน ' . $processed_count . ' รายการ', null);
