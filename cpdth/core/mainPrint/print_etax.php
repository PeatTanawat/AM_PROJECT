...
try {
    ...
    // ตรวจสอบสิทธิ์ว่า user คนนี้เป็นเจ้าของใบกำกับภาษีนี้จริง
    ...
    $etx_row = $stmt->fetch(PDO::FETCH_ASSOC);
    $stmt->closeCursor();

    if (!$etx_row) {
        Response::json(0, 'ไม่พบข้อมูลใบกำกับภาษีนี้ หรือคุณไม่มีสิทธิ์เข้าถึง', null);
    }

    // เรียกใช้ Autoloader และคลาส Pdf จากฝั่ง backoffice เฉพาะสำหรับตัว mPDF Library เท่านั้น (เพื่อให้วาด PDF ได้)
    ...
    // ==========================================
    // แยกส่วนการสร้างหน้าตา PDF มาไว้ที่ฝั่ง cpdth โดยเฉพาะ
    // ==========================================
    if (!function_exists('pdf_baht_text_cpdth')) {
        function pdf_baht_text_cpdth(float $amount): string
        {
    ...
    // ดึงข้อมูลคำสั่งซื้อและรูปบัตรประชาชน
    $stmt = $pdo_connect->prepare(
        "SELECT o.order_id, o.user_id, o.total_price, o.payment_method, o.created_at,
                ...
                u.user_firstname, u.user_lastname, u.user_phone, u.id_card_image, u.user_citizen_id
            FROM tbl_orders o
    ...
    $user_citizen = $order['user_citizen_id'] ?? '';
    if (empty($user_citizen)) {
        // Fallback in case user_citizen_id is not in $order
        $stmt_cit = $pdo_connect->prepare("SELECT user_citizen_id FROM tbl_user WHERE user_id = :uid LIMIT 1");
        $stmt_cit->execute([':uid' => (int) $order['user_id']]);
        $user_citizen = $stmt_cit->fetchColumn() ?: '';
    }
    ...
    if ($etx_row) {
        ...
        $doc_no = $etx_row['etax_no'] ?: 'ET' . date('ym', $ts) . str_pad((string) $order['order_id'], 7, '0', STR_PAD_LEFT);
    ...
    // โลโก้ AM GROUP
    $logo_path = dirname(__DIR__, 2) . '/assets/images/logo/am-group-logo.png';
    ...
    if ($items) {
        ...
        $coupon_desc = 'ส่วนลด';
        try {
            // ใช้ JOIN เพื่อดึงข้อมูลจาก log และรายละเอียดคูปองพร้อมกัน
            $stmt_cl = $pdo_connect->prepare("
                SELECT cl.*, c.* 
            ...
            if ($cop_log) {
                // หาจำนวนเงินส่วนลด
                $discount = (float) ($cop_log['discount'] ?? $cop_log['discount_amount'] ?? $cop_log['amount'] ?? $cop_log['price'] ?? $cop_log['coupon_discount'] ?? $cop_log['coupon_min'] ?? 0);

                // หาชื่อ/โค้ดคูปอง
                $code = $cop_log['coupon_code'] ?? $cop_log['coupon_name'] ?? $cop_log['coupon_detail'] ?? '';
                if ($code !== '') {
                    $coupon_desc .= ' (' . $code . ')';
                }
            ...
            // โชว์ error ชั่วคราวถ้า column ผิด
            $rows .= '<tr><td colspan="5" align="center" style="color:red; font-size:9pt;">[Debug] ' . $esc($e->getMessage()) . '</td></tr>';
        }
    ...
    // แทรกรูปบัตรประชาชน (ถ้ามี)
    $id_card_img_html = '';
    if (!empty($order['id_card_image'])) {
        $id_card_path = dirname(__DIR__, 2) . '/' . ltrim($order['id_card_image'], '/');
        if (file_exists($id_card_path)) {
            $id_card_img_html = '<div style="margin-top:15px;"><img src="' . $id_card_path . '" style="max-width:240px; max-height:120px; border:1px solid #ccc; border-radius:4px;"></div>';
        }
    }
    ...
    <div align="center" style="margin-top:24px;font-weight:bold;">' . $esc(pdf_baht_text_cpdth($total)) . '</div>
