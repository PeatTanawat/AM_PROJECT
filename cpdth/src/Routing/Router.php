...
class Router
    {
        ...
        public function dispatch(string $state, string $action): void
        {
            ...
            if (!isset($this->routes[$key])) {
                $debug_info = "Method: " . $_SERVER['REQUEST_METHOD'] . ", URI: " . $_SERVER['REQUEST_URI'] . ", POST: " . json_encode($_POST) . ", RAW: " . file_get_contents('php://input');
                Response::json(9, 'Route not found | Debug: ' . $debug_info);
            }
            ...
            if (!is_string($handler) || !is_file($handler)) {
                Response::json(9, 'Handler file not found: ' . $handler);
            }
