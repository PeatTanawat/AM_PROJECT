...
// ครั้งแรกที่ออกใบ (ต่อ enroll_id) จะ "freeze" ข้อมูล ณ วันนั้นลง tbl_certificate_snapshot
...
if (! $snap) {
    ...
    // รหัสหลักสูตร CPD และ CPA สำหรับบันทึก
    $cpd_raw = (string) ($row['course_code_cpd_' . $q_idx] ?? ($row['course_code_cpd_1'] ?? ''));
    $cpd_q   = $cpd_raw !== '' ? preg_replace('/\[\d{1,2}\](?=[^\[]*$)/', $quarter, $cpd_raw, 1) : '';
    $ccode_cpd_frozen = $cpd_q !== '' ? str_replace(['[', ']'], '', $cpd_q) : '';

    $cpa_raw = (string) ($row['course_code_cpa_' . $q_idx] ?? ($row['course_code_cpa_1'] ?? ''));
        ...
        if ($ex->getCode() !== '23000') {
            try {
                ...
                if ($ex2->getCode() !== '23000') {
                    ...
                    Response::json(0, 'สร้างใบรับรองไม่สำเร็จ: ' . $ex2->getMessage(), null);
                }
            }
        }
    }
...
$course_cpd_code = isset($snap['course_code_cpd']) ? (string)$snap['course_code_cpd'] : '';
$course_cpa_code = isset($snap['course_code_cpa']) ? (string)$snap['course_code_cpa'] : '';
$cpd_hours_text = '-';
$cpa_hours_text = '-';
...
if ($course_row) {
    // รหัสหลักสูตร CPD (ถ้ายังไม่มีใน snapshot ให้ใช้ค่าคำนวณจาก course_row)
    if ($course_cpd_code === '') {
    ...
    // รหัสหลักสูตร CPA (ถ้ายังไม่มีใน snapshot ให้ใช้ค่าคำนวณจาก course_row)
    if ($course_cpa_code === '') {
...
if ($user_cpd_no !== '' && $user_cpa_no !== '') {
    ...
    $license_rows_html .= '<tr><td align="right" class="lbl">เลขที่ผู้ทำบัญชี</td><td style="font-size:18px;">:&nbsp;' . $esc($user_cpd_no) . '</td></tr>';
    $license_rows_html .= '<tr><td align="right" class="lbl">ชื่อหลักสูตร</td><td style="font-size:18px;">:&nbsp;' . $esc($snap['course_name'] ?? '-') . '</td></tr>';
    $license_rows_html .= '<tr><td align="right" class="lbl" style="padding-top: 15px;">รหัสหลักสูตร</td><td style="font-size:17px; padding-top: 15px;">:&nbsp;' . $esc($course_cpd_code !== '' ? $course_cpd_code : '-') . '</td></tr>';
    
    $approve_date = (! empty($snap['course_approval_date']) && $snap['course_approval_date'] !== '0000-00-00')
        ? date('d/m/Y', strtotime($snap['course_approval_date'])) : '-';
        
    $license_rows_html .= '<tr><td align="right" class="lbl">วันที่อนุมัติหลักสูตร</td><td style="font-size:17px;">:&nbsp;' . $esc($approve_date) . '</td></tr>';
    $license_rows_html .= '<tr><td align="right" class="lbl">นับชั่วโมงด้าน</td><td style="font-size:17px;">:&nbsp;' . $esc($cpd_hours_text) . ' สำหรับผู้ทำบัญชี</td></tr>';
} elseif ($user_cpa_no !== '') {
    $license_rows_html .= '<tr><td align="right" class="lbl">เลขที่ผู้สอบบัญชี</td><td style="font-size:18px;">:&nbsp;' . $esc($user_cpa_no) . '</td></tr>';
    $license_rows_html .= '<tr><td align="right" class="lbl">ชื่อหลักสูตร</td><td style="font-size:18px;">:&nbsp;' . $esc($snap['course_name'] ?? '-') . '</td></tr>';
    $license_rows_html .= '<tr><td align="right" class="lbl" style="padding-top: 15px;">รหัสหลักสูตร</td><td style="font-size:17px; padding-top: 15px;">:&nbsp;' . $esc($course_cpa_code !== '' ? $course_cpa_code : '-') . '</td></tr>';
    
    $approve_date = (! empty($snap['course_approval_date']) && $snap['course_approval_date'] !== '0000-00-00')
        ? date('d/m/Y', strtotime($snap['course_approval_date'])) : '-';
        
    $license_rows_html .= '<tr><td align="right" class="lbl">วันที่อนุมัติหลักสูตร</td><td style="font-size:17px;">:&nbsp;' . $esc($approve_date) . '</td></tr>';
    $license_rows_html .= '<tr><td align="right" class="lbl">นับชั่วโมงด้าน</td><td style="font-size:17px;">:&nbsp;' . $esc($cpa_hours_text) . ' สำหรับผู้สอบบัญชี</td></tr>';
} else {
    $fallback_lic       = trim((string) ($snap['user_citizen_id'] ?? ''));
    $license_rows_html .= '<tr><td align="right" class="lbl">เลขประจำตัวผู้ทำบัญชี</td><td style="font-size:18px;">:&nbsp;' . $esc($fallback_lic !== '' ? $fallback_lic : '-') . '</td></tr>';
    $license_rows_html .= '<tr><td align="right" class="lbl">ชื่อหลักสูตร</td><td style="font-size:18px;">:&nbsp;' . $esc($snap['course_name'] ?? '-') . '</td></tr>';
    $license_rows_html .= '<tr><td align="right" class="lbl" style="padding-top: 15px;">รหัสหลักสูตร</td><td style="font-size:17px; padding-top: 15px;">:&nbsp;' . $esc($course_cpd_code !== '' ? $course_cpd_code : ($course_cpa_code !== '' ? $course_cpa_code : '-')) . '</td></tr>';
...
$sign_uri  = Pdf::fileToDataUri(dirname(__DIR__, 3) . '/assets/images/signature-amgroup.jpg');
if ($sign_uri === '') {
