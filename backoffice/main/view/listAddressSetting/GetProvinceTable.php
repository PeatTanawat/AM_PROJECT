<?php
// ตั้งค่าที่อยู่ — view fragment: render ตารางระดับจังหวัด
...
<div class="default-table-area">
    <div class="table-responsive">
        <table class="table align-middle w-100" id="ProvinceTable" style="table-layout: fixed;">
            <thead>
                <tr>
                    <th scope="col" class="text-center" style="width: 8%;">ลำดับ</th>
                    <th scope="col" class="text-start" style="width: 32%;">จังหวัด</th>
                    <th scope="col" class="text-center" style="width: 18%;">จำนวนอำเภอ</th>
                    <th scope="col" class="text-center" style="width: 18%;">จำนวนตำบล</th>
                    ...
                    <?php $n = $from; foreach ($list as $row): ?>
                        <tr>
                            ...
                            <td class="text-center">
                                <span class="fw-medium text-dark"><?php echo number_format((int)($row['district_count'] ?? 0)); ?> อำเภอ</span>
                            </td>
                            <td class="text-center">
                            ...
                            <td class="text-center">
                                <div class="d-flex gap-1 justify-content-center">
                                    <button type="button" 
                                            ...
                                            onclick="SelectProvince(<?php echo (int)$row['id']; ?>, '<?php echo $esc(addslashes($row['name_th'] ?? '')); ?>')">
                                    ...
                                    <button type="button" 
                                            ...
                                            title="แก้ไข"
                                            onclick="GetEditProvince(<?php echo (int)$row['id']; ?>, '<?php echo $esc(addslashes($row['name_th'] ?? '')); ?>', '<?php echo $esc(addslashes($row['name_en'] ?? '')); ?>')">
                                    ...
                                    <button type="button" 
                                            ...
                                            title="ลบ"
                                            onclick="DeleteProvince(<?php echo (int)$row['id']; ?>, '<?php echo $esc(addslashes($row['name_th'] ?? '')); ?>')">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="100" class="text-center py-5">
                            ...
                            <div class="list-empty-title text-muted">ไม่พบข้อมูลจังหวัด</div>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
