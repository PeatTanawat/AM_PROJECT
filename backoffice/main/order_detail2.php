        ...
        <div class="px-2">
            ...
            <div class="d-flex flex-wrap justify-content-end align-items-center gap-2 mb-3">
                <div class="d-flex flex-wrap gap-2">
                    <a href="order_pending.php" class="btn btn-outline-secondary d-inline-flex align-items-center gap-1">
                    <span class="material-symbols-outlined" style="font-size: 18px;" aria-hidden="true">arrow_back</span> กลับ
                </a>
                    <button type="button" id="BtnConfirmPayment" class="btn btn-success d-inline-flex align-items-center gap-1" onclick="ConfirmPayment()">
                        <span class="material-symbols-outlined" style="font-size:18px;" aria-hidden="true">check_circle</span> อนุมัติ
                    </button>
                    <button type="button" id="BtnCancelOrder" class="btn btn-danger d-inline-flex align-items-center gap-1" onclick="CancelOrder()">
                        <span class="material-symbols-outlined" style="font-size:18px;" aria-hidden="true">cancel</span> ปฏิเสธ
                    </button>
            ...
            <div class="row g-4">
                <!-- กล่องด้านซ้าย: ข้อมูลออเดอร์ & รายการสินค้า -->
                <div class="col-lg-7">
                    <!-- 1. ข้อมูลทั่วไป -->
                    ...
                    <div class="card app-card bg-white border-0 rounded-3 mb-4">
                        <div class="card-body p-4">
                            <div class="od-section-title mb-3">
                                <span class="material-symbols-outlined" aria-hidden="true">shopping_cart</span> รายการสินค้าที่ซื้อ
                            </div>
                ...
                <!-- กล่องด้านขวา: สลิปโอนเงิน -->
                <div class="col-lg-5">
                    <div class="card app-card bg-white border-0 rounded-3 h-100" id="SectionSlipImage" style="display:none;">
                        <div class="card-body p-4 text-center">
                            <div class="od-section-title mb-3 justify-content-center">
                                <span class="material-symbols-outlined" aria-hidden="true">image</span> สลิปโอนเงิน
                            </div>
                            <div class="mb-3">
                                <a id="OrdSlipLink" href="#" target="_blank" title="คลิกเพื่อดูรูปภาพขนาดเต็ม" style="display: inline-block;">
                                    <img id="OrdSlipImage" src="" alt="Payment Slip" class="img-fluid border rounded shadow-sm" style="max-width: 100%; height: auto; max-height: 700px; object-fit: contain; cursor: pointer;">
                                </a>
                            </div>
                            <!-- <div>
                                <a id="OrdSlipBtn" href="#" target="_blank" class="btn btn-sm btn-outline-primary d-inline-flex align-items-center gap-1">
                                    <span class="material-symbols-outlined" style="font-size: 16px;">open_in_new</span> ดูรูปภาพขนาดเต็ม
                                </a>
                            </div> -->
                        </div>
                    </div>
                    
                    <div class="card app-card bg-white border-0 rounded-3 h-100" id="SectionNoSlip" style="display:none;">
                        <div class="card-body p-4 text-center d-flex flex-column justify-content-center align-items-center text-muted" style="min-height: 300px;">
                            <span class="material-symbols-outlined mb-2" style="font-size:48px;" aria-hidden="true">image_not_supported</span>
                            <div>ไม่พบรูปภาพสลิปโอนเงิน</div>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function RenderOrder(d) {
        ...
        console.log("Order Data:", o); // Debug ข้อมูลออเดอร์
        ...
        // ปุ่มยืนยัน/ยกเลิก แสดงเฉพาะออเดอร์ที่รอยืนยัน
        ...
        var canConfirm = (o.payment_status === "0" && (o.payment_method === "2" || o.payment_method === "3"));
    ...
    function formatSlipImageUrl(path) {
        if (!path) return "";
    ...
    // แสดงสลิปโอนเงิน (ถ้ามี)
    if (o.slip_image) {
        var slipUrl = formatSlipImageUrl(o.slip_image);
        $("#SectionSlipImage").show();
        $("#SectionNoSlip").hide();
        $("#OrdSlipImage").attr("src", slipUrl);
        $("#OrdSlipLink").attr("href", slipUrl);
        $("#OrdSlipBtn").attr("href", slipUrl);
    } else {
        $("#SectionSlipImage").hide();
        $("#SectionNoSlip").show();
    }
    ...
    function ConfirmPayment() {
        Swal.fire({
            ...
            $.ajax({
                ...
                data: {
                    request_state: "list_order",
                    request_function: "confirm_payment",
                    order_id: ORDER_ID
                },
                dataType: "json",
                success: function (res) {
    ...
    function CancelOrder() {
        Swal.fire({
            ...
            html: '<span class="text-secondary">ต้องการยกเลิกคำสั่งซื้อนี้ใช่หรือไม่? โปรดระบุหมายเหตุการยกเลิก</span>',
            icon: "warning",
            input: "textarea",
            inputPlaceholder: "ใส่หมายเหตุการยกเลิกที่นี่...",
            showCancelButton: true,
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.result == 1) {
                        ...
                        Swal.fire({ 
                            title: "แจ้งเตือน", 
                            html: '<span class="fw-bold text-danger">' 
                                    + res.msg 
                                    + '</span>', 
                            icon: "error", 
                            showConfirmButton: true 
